...
function validateJobBoard({ profile, companies, listings }) {
  ...
  for (const company of companies) {
    ...
    assert.ok(["ashby", "greenhouse", "careers"].includes(company.source.type), "Unsupported company source");
    if (["ashby", "greenhouse"].includes(company.source.type)) assert.ok(/^[a-z0-9-]+$/.test(company.source.board), "Invalid employer board");
  ...
  for (const job of listings) {
    ...
    if (job.lastCheckedAt) assert.ok(isDate(job.lastCheckedAt) && job.lastCheckedAt >= job.firstSeenAt, "Invalid latest check date");
    if (job.status === "closed") {
      assert.ok(isDate(job.archivedAt) && job.archivedAt >= job.firstSeenAt, "Archived roles require a valid archive date");
      assert.ok(job.archiveReason?.trim(), "Archived roles require a reason");
    }
    for (const entry of job.availabilityHistory || []) {
      assert.ok(isDate(entry.date), "Availability evidence needs a date");
      assert.ok(["open", "closed", "missing", "error"].includes(entry.outcome), "Invalid observation outcome");
      assert.ok(entry.detail?.trim(), "Availability evidence needs details");
      checkUrl(entry.sourceUrl);
    }
  }
  return true;
}
