...
function filterJobs(listings, companies, { query = "", track = "all", companyId = "all", sort = "priority", status = "active" } = {}) {
  ...
  return listings.filter((job) => {
    ...
    return (status === "archived" ? job.status === "closed" : job.status !== "closed") && (track === "all" || job.track === track)
